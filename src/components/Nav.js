...
const Nav = () => {
  ...
  return (
    <nav className='shadow-md sticky top-0 left-0 right-0 bg-white'>
      <div className='flex max-w-screen-xl items-center justify-between mx-auto p-4'>
        ...
        <div className={`${!open && "hidden"} bg-gray-600/50 min-h-screen w-full fixed top-0 left-0 right-0 backdrop-blur-sm z-[2]`} onClick={() => setOpen(false)}></div>

        <div className={`${open ? "w-80 z-[3]" : "w-0"} bg-[#fafbfd] min-h-screen fixed top-0 left-0 transition-all duration-300 z-[4]`}>
